class BirthdayCard {
    constructor() {
        ...
        this.videoCleanupHandlers = null;
    ...
    startVideoSequence() {
        ...
        if (!videoOverlay || !video) {
            console.warn('Video elements not found, skipping to card');
        ...
        this.videoCleanupHandlers = { onVideoEnded, onSkip };
        
        // Запускаем видео
        video.play().catch(e => {
            console.log('Autoplay prevented:', e);
            // Показываем подсказку для ручного запуска (браузеры могут блокировать)
            const hint = document.querySelector('.video-hint');
            if (hint) hint.style.opacity = '0.8';
        });
    }
    
    finishVideoAndShowCard() {
        const videoOverlay = document.getElementById('videoOverlay');
        const video = document.getElementById('birthdayVideo');
        const skipBtn = document.getElementById('skipVideoBtn');
        
        if (this.videoCleanupHandlers && video && skipBtn) {
            video.removeEventListener('ended', this.videoCleanupHandlers.onVideoEnded);
            skipBtn.removeEventListener('click', this.videoCleanupHandlers.onSkip);
            this.videoCleanupHandlers = null;
        }
    ...
    transitionToCard() {
        ...
        setTimeout(() => {
            this.showMessage("🎉 С Днём Рождения, Орхан!");
        }, 500);
